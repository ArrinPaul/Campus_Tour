...
export const Menu: React.FC = () => {
  ...
  const handleHistoryClick = (imageId: string) => {
    ...
    for (const block of manifest.blocks) {
      const found = block.labs.find((lab: Lab) => lab.id === imageId);
      if (found) {
        setBlock(block.id);
        setImage(imageId);
        break;
  ...
  const historyItems: HistoryItem[] = history
    .map((imageId: string) => {
      for (const block of manifest.blocks) {
        const found = block.labs.find((lab: Lab) => lab.id === imageId);
        if (found) return { imageId, label: found.id, blockLabel: block.name };
      }
      return null;
    })
  ...
  return (
    <div className="absolute left-6 top-1/2 -translate-y-1/2 flex flex-col gap-6 max-h-[85vh] w-80">
      {/* Search Bar */}
      ...
      {history.length > 0 && (
        <motion.button
          ...
          className="flex items-center gap-3 px-5 py-3.5 bg-gradient-to-r from-indigo-500/20 to-purple-500/20 backdrop-blur-xl border border-white/20 rounded-2xl text-white/80 hover:text-white hover:border-white/30 transition-all duration-300 shadow-lg hover:shadow-[0_0_25px_rgba(102,126,234,0.4)]"
        >
          <Clock size={18} />
      ...
      {showHistory && historyItems.length > 0 && (
        ...
        >
          <p
            ...
            className="text-xs text-white/60 uppercase tracking-wider font-semibold mb-1"
          >
            Recent Visits
          </p>
          {historyItems.map((item: HistoryItem, idx: number) => (
            <motion.button
              ...
              aria-label={`Go to ${item.blockLabel} - ${item.label}`}
      ...
      <div className="flex flex-col gap-4 overflow-y-auto pr-2 scrollbar-hide">
        {filteredBlocks.map((block: Block) => (
          ...
          >
            <div className="relative z-10">
              <h3 className="font-bold text-lg mb-1" style={{ letterSpacing: '-0.02em' }}>
                {block.name}
              </h3>
            ...
            {/* Gradient Overlay */}
            <div
              className={`absolute inset-0 rounded-2xl bg-gradient-to-r from-indigo-500/20 via-purple-500/20 to-pink-500/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none`}
            />
        ...
        {filteredBlocks.length === 0 && (
          <div className="text-white/50 text-center py-8 text-sm">No locations found</div>
        )}
      </div>
    </div>
  );
